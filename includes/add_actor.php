<?php
  ...
  if (isset($_POST['submit_actor'])){
      ...
      if(record_exits($query_actor)){
        $_SESSION["error"] = "<p class='bg-danger'>This actor already exists in the database.</p><br/>";
        $form_errors = true;
      }
      ...
      if($form_errors == false){
        ...
        $_SESSION["message"] = "<p class='bg-success'>New actor successfully added to database</p>";
        $first_name = "";
        $last_name = "";
      }
   ...
   <form action="<?php htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post" class="form-horizontal">
    ...
    <div class="form-group">
      <div class="col-sm-offset-2 col-sm-4">
        <input id="submit" type="submit" name="submit_actor" value="Submit" class="btn btn-success" />
      </div>
    </div>
  </form>
 </div>
